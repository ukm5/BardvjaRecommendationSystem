...
df = make_training_dataframe('./../papers_train/')
...
df.to_csv('./../training_data/local_papers.csv', index=False)

df = pd.read_csv('./../training_data/local_papers.csv')
...
def transform_abstracts_to_vectors(df, custom_stop_words=['a']):
    ...
    try:
        joblib.dump(stop_words_english, './../trained_models/stop_words_english')
    except:
    ...
    joblib.dump(cvec.vocabulary_, './../trained_models/cvec_vocabulary')
    # Save the Vectorizer models for later
    joblib.dump(cvec, './../trained_models/count_vectorizer.sav')
    joblib.dump(tvec, './../trained_models/tfidf_vectorizer.sav')
    
    ss = StandardScaler()
    X_vec_ss = ss.fit_transform(X_vec.toarray())
    # Save the StandardScaler for later
    joblib.dump(ss, './../trained_models/standard_scaler.sav')
    
    k_cluster = KMeans(n_clusters=3, random_state=42, n_jobs=-1)
    k_cluster.fit(X_vec_ss)
    # Save the KMeans cluster for later
    joblib.dump(k_cluster, './../trained_models/kmeans_cluster.sav')
...
sparse.save_npz('./../training_data/normalized_train_vectors.npz', matrix=Z_sparse)

print("\n ***************** Exiting the code where local pdf files are parsed and vectorized **************** \n")
